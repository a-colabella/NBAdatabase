...
def runAddPlayer(connection, update, rt, entry):
    ...
    name_entry = Entry(add_window, textvariable=nameVar).grid(row=0, column=2)
    pos_label = Label(add_window, text="Position: ").grid(row=1, column=1)
    pos_entry = Entry(add_window, textvariable=posVar).grid(row=1,column=2)
    ...
    team_entry = Entry(add_window, textvariable=teamVar).grid(row=3,column=2)
    ...
    submit_button = Button(add_window, text="Submit").grid(row=15, column=2)

    if update:
        add_window.title('Update Player')
        id_label = Label(add_window, text="ID: ").grid(row=14, column=1)
        id_val = Label(add_window, text="insert").grid(row=14, column=2)
...
def runAddCoach(connection, update, rt, entry):
    ...
    team_entry = Entry(add_window, textvariable=teamVar).grid(row=1, column=2)
    submit_button = Button(add_window, text="Submit").grid(row=3, column=2)

    if update:
        add_window.title('Update Coach')
        id_label = Label(add_window, text="ID: ").grid(row=2, column=1)
        id_val = Label(add_window, text="insert").grid(row=2, column=2)
        nameVar.set(entry[0] + " " + entry[1])
        teamVar.set(entry[2])
    else:
